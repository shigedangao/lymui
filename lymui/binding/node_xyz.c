...
napi_value GetRGBFromXyz(napi_env env, napi_callback_info info) {
    ...
    napi_value object;
    // str
    char * enumStr = malloc(sizeof(char) * XYZ_MATRIX_LEN + 1);
    size_t str;
    ...
    status = napi_get_value_string_utf8(env, argv[1], enumStr, XYZ_MATRIX_LEN, &str);
    if (status != napi_ok) {
        napi_throw_error(env, NULL, PARSE_ERR);
        return NULL;
    }
    
    Xyz * xyz = getXyzFromJSObj(env, argv[0]);
    if (xyz == NULL) {
        napi_throw_error(env, NULL, PROP_NOT_FOUND_ERR);
        return NULL;
    }
    
    Matrix m  = getEnumFromStr(enumStr);
    Rgb * rgb = generateRgbFromXyz(xyz, m);
